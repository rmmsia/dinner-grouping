...
class GreedyGroupManager:
    ...
    def _patch_pairing_scores(self, groups: List[List[str]], patch_value: float):
        for group in groups:
            for i, person1 in enumerate(group):
                for person2 in group[i+1:]:
                    self.pairing_scores.at[person1, person2] = patch_value
                    self.pairing_scores.at[person2, person1] = patch_value
